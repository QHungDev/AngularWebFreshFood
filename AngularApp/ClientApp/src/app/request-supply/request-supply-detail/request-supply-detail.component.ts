...
@Component({
  selector: 'app-request-supply-detail',
  templateUrl: './request-supply-detail.component.html',
  styleUrls: ['./request-supply-detail.component.css']
})
export class RequestSupplyDetailComponent implements OnInit {
  ...
  postRequestSupply: any = {
    ID: 0,
    ProductID: "",
    CreateBy: "",
    CreateTime: new Date(),
    Supplier: "FreshFood",
    QuantityRequestSupply: 0,
    UnitPrice: 0,
    Total: 0,
    Status: false,
  };
  ...
  onChange() {
    this.postRequestSupply.Total = this.postRequestSupply.QuantityRequestSupply * this.postRequestSupply.UnitPrice
  }
  ...
  submitForm() {
    if (!this.productID || this.productID == "" ||
        !this.postRequestSupply.QuantityRequestSupply || this.postRequestSupply.QuantityRequestSupply == "" || this.postRequestSupply.QuantityRequestSupply == 0 ||
        !this.postRequestSupply.UnitPrice || this.postRequestSupply.UnitPrice == "" || this.postRequestSupply.UnitPrice == 0 ||
        !this.postRequestSupply.Total || this.postRequestSupply.Total == "" || this.postRequestSupply.Total == 0) {

          alert("Vui lòng nhập đầy đủ thông tin!");
          return
    }
    this.postRequestSupply.ProductID = this.productID
    this.postRequestSupply.CreateBy = window.localStorage.getItem('Username');
    console.log(this.postRequestSupply)
    this.productService.addRequestSupply(this.postRequestSupply).subscribe({
      next: (data => {
        alert("Gửi yêu cầu nhập kho thành công!");
        this.router.navigate(['/request-supply']);
      }),
